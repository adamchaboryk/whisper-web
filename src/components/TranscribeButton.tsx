...
import { TranscriptionRecovery } from "../hooks/useTranscriber";
  ...
  function TranscribeButton(props: Props, ref) {
    ...
    return (
      <button
        ...
        className='transcribe-button text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center'
      >
        {recovery ? (
          <Spinner
            text={
          ...
          <Spinner
            text={
              modelLoadingProgress !== undefined
                ? `Loading model (${modelLoadingProgress.toFixed(0)}%)...`
                : "Loading model..."
            }
          />
        ) : isTranscribing ? (
          <Spinner
            text={
              transcribingProgress !== undefined
                ? `Transcribing (${Math.min(transcribingProgress, 99).toFixed(0)}%)...`
                : "Transcribing..."
            }
          />
        ) : (
